<template>
  <div>
    <multiselect v-model="selection" :options="options" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Select invoice items" label="description" track-by="description" :preselect-first="true">
      <template slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="selection.length &amp;&amp; !isOpen">{{ selection.length }} options selection</span></template>
    ...
    <ul class="tail-mt-4 tail-p-4 tail-pb-3 tail-border-gray-400 tail-border-solid tail-border tail-rounded-lg tail-border-opacity-30">
      <li v-for="(select,index) in selection" class="tail-flex tail-justify-between tail-mb-3" :key="index"> 
          <div>
          ...
          <div class="tail-flex tail-justify-between tail-items-center tail-gap-2">
            <h5 class="tail-font-medium tail-text-lg"> {{ select.price | amount }}</h5>
    ...
    <Modal :is-open="openEditItem" status="Edit Item" @close="openEditItem = $event">
