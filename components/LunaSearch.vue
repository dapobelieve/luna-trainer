  ...
  >
    <div class="">
      <div class="search-header rounded-t-lg  px-4 pt-4">
        <div class="border rounded-lg mb-5 px-3">
          <form @submit.prevent="" role="search" novalidate class="flex items-center justify-center">
            ...
            <button @click.prevent="query=''" class="cancel text-sm text-black px-1 font-bold text-xs" style="font-size: 10px">
      ...
      <div class="search-body h-full max-h-[28rem] md:max-h-[35rem] overflow-y-auto">
        <div class="px-4 mb-6">
          <p class="text-gray-600">Showing all results for <span class="text-gray-900 font-bold">{{query}}</span></p>
        </div>
        <div v-if="items.length > 0" class="search-items">
          <ul class="list-none">
            <div class="mb-4 ml-4 text-sm uppercase leading-5">
              Clients
            </div>
            <LunaSearchItem v-for="x in 2" />
          </ul>
        </div>
